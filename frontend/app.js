...
class AgentChat {
    ...
    async checkBackendStatus() {
        try {
            // Use the URL from input field if available
            const backendUrl = this.backendUrlInput ? this.backendUrlInput.value || 'http://localhost:8002' : BACKEND_URL;
    ...
    getBackendUrl() {
        return this.backendUrl || (this.backendUrlInput ? this.backendUrlInput.value : null) || BACKEND_URL;
    }
...
function connectToBackend() {
    if (window.agentChat && window.agentChat.backendUrlInput) {
        const url = window.agentChat.backendUrlInput.value.trim();
        if (url) {
            window.agentChat.backendUrl = url;
            window.agentChat.checkBackendStatus();
        }
    }
}
